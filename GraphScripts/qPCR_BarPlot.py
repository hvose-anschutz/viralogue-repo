...
TissueOrder = ["Mat - Deep Well","mLN","Omentum","Prox. Colon","PP"]	       #A list of labels 
...
TreatmentOrder = ["Uninfected - 3dpi","Uninfected - 5dpi","Infected - 3dpi","Infected - 5dpi"]
...
Colors.add(["Uninfected - 3dpi","Uninfected - 5dpi","Infected - 3dpi","Infected - 5dpi"])
custom_colors.add(["Uninfected - 3dpi","Uninfected - 5dpi","Infected - 3dpi","Infected - 5dpi"],["#AE3899","#CF92DD","#009933","#EDAB21"])

WhiteWheel = myfunc.get_white_wheel(DataSet,"Sample Name") #specify dataset loaded in and the variable to match colors to
...
g = sns.catplot(data=DataSet,
    ...
    x="Sample ID",                  #x axis data
    y="CT",             #y axis data
    hue="Sample Name",	            #defines how the bars will be split
    ...
    palette= custom_colors.colors,            #controls the colors on the graph, each value of Treatment must have a hex code
...
ax = sns.swarmplot(data=DataSet, 
					x="Sample ID",                 #x axis data
					y="CT",             #y axis data
					hue="Sample Name",            #category separating the data by color
...
plt.show()
# plt.savefig(mySVGOut,bbox_inches="tight",dpi=300)          #saves the file to the generated_images folder