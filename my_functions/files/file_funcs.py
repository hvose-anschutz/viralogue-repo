...
def tab_separator(my_file: str, delimiter: str="\t",header: bool=False) -> list[str]:
    """Separates a delimited file into a list object. The delimiter can be specified, but tab is the default."""
    return_list = []
...
def write_dict_to_file(my_filename: str, my_dict: dict, delimiter: str="\t", keep_flags: bool=False):
    """Outputs the contents of a dictionary to a specified file.\n
    my_filename: the desired filename (if no path specified, it will output to the current directory)\n
